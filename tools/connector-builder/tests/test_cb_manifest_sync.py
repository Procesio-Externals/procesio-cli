"""Guard: tool.yaml (source of truth) declares exactly the actions the dispatcher
registers, every CLI flag appears as a manifest arg (and vice-versa), required
flags are marked required, and the three auth secrets are declared.

``main`` comes in via the ``cb_main`` alias from conftest."""
...
def test_secrets_declared():
    m = _manifest()
    names = {s.name for s in m.secrets}
    assert names == {"api-key", "username", "password"}
