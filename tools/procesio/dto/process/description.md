...
- **Query Store node** (`{"action":"Query Store","queryStore":{"sql":…,"params":{…},"timeout":…,"resultRows":…,"totalCount":…}}`) — ONE MySQL statement over the workspace's Data Stores (SELECT/WITH/JOIN/GROUP BY plus INSERT/UPDATE/DELETE/REPLACE; reads AND writes). In `sql`, write a table as `{{ds:StoreNameOrId}}` and a column as `{{col:StoreNameOrId.ColNameOrId}}` (each becomes an id-bound chip resolved live from Data Store metadata, or pass a GUID), a parameter as `@name` (bound via `params`, injection-safe), and system columns (`CreatedOn`/`CreatedById`/…) by plain name. `params` maps each `@name` -> a binding (bare = LITERAL; `{var[,path]}`/`{value}` otherwise). `timeout` 60-1800 (default 1800). `resultRows` -> a **list-of-Json** variable, `totalCount` -> an **Integer** (rows returned, or rows affected for a write). INSERT/REPLACE must set `CreatedById`+`UpdatedById` (a user GUID param); a LIST param uses `JSON_CONTAINS(@l, JSON_QUOTE(col))` / `col MEMBER OF (CAST(@l AS JSON))`, never `IN (@l)`. Verified live end-to-end. Full contract: `PROCESIO-QUERY-STORE-NOTES.md`.
