...
--  - Triggers (DML + database DDL); server triggers are a separate opt-in
-- Scope: whole current database, including confidential definitions/literals.
-- Obtain scope/confidentiality approval and protect output BEFORE acquisition.
-- Legacy inspection aid only; generated text is not qualified for automatic replay.
-- NULL trigger definitions can propagate into @Def; missing output is not absence.
...
DECLARE @p_Database   SYSNAME        = DB_NAME(); -- output label only; does not switch database
-- Server-wide definitions exceed database scope: set to 1 only after separate approval.
DECLARE @p_IncludeServerTriggers BIT = 0;
...
--------------------------------------------------------------------------------
-- DDL TRIGGERS (database-level) + SERVER TRIGGERS (opt-in and if permitted)
--------------------------------------------------------------------------------
;WITH DDLT AS
(
    SELECT
...
-- Preserve the legacy CONTROL SERVER check, but permission alone is not scope approval.
IF @p_IncludeServerTriggers = 1 AND HAS_PERMS_BY_NAME(NULL, NULL, 'CONTROL SERVER') = 1
BEGIN
    ;WITH ST AS
    (
        SELECT
...
ELSE
BEGIN
    SET @Def += N'-- NOTE: Skipped server-level triggers (opt-in disabled or no CONTROL SERVER permission).' + @CRLF + @CRLF;
END;
