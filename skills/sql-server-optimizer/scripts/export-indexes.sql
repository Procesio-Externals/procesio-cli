/*Optimized SQL:*/
-- Legacy inspection-only exporter over all user-table indexes in the current DB.
-- Obtain whole-database scope/confidentiality approval BEFORE acquisition; filter
-- literals and names may be sensitive. Protect both result and message output.
-- The generic serializer is NOT correct for all types; do not automatically run
-- on mixed/unsupported types (columnstore, XML, spatial, hash, other non-rowstore).
-- Use a reviewed bounded catalog projection instead. No generated DDL is certified
-- complete or safe to execute, even for rowstore; query semantics are retained.
-- Builds one NVARCHAR(MAX) blob and prints it in SSMS-sized chunks.
-- Notes:
--  - Excludes indexes that back PRIMARY KEY or UNIQUE constraints (already in export-tables.sql).
--  - The query selects non-rowstore indexes too; this is NOT serialization support.
...
SET @Def += N'-- Database indexes export' + @CRLF
         ...
         +  N'-- Inspection only: not qualified DDL; unsupported types may be invalid or missing' + @CRLF
         +  N'-- Query scope: All non-constraint indexes on user tables' + @CRLF
...
- PK and UQ constraint-backed indexes are excluded — they are already scripted in export-tables.sql.
  Use the outputs as partial inspection context; verify against a bounded catalog projection.
...
- fill_factor = 0 is omitted from WITH clause; verify the server default independently.
- Disabled indexes include CREATE INDEX + ALTER INDEX ... DISABLE text for inspection;
  identical recreation on another instance is not guaranteed or qualified.
