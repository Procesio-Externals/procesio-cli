...
- **Expect small gaps.** Two files carry regions that the export drops. If a
  document refers to something you can't find, that's why. Say so in an issue
  and a maintainer fixes the reference.
...
the same license as this repository. See [LICENSE](LICENSE).
